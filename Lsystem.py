...
surface.fill((0,0,0))
class Instructions:
    def __init__(self,start,rules,angle,interactions):
...
def gerar(instructions,line_size,last = (400,400,0)):
    ...
    for inst in range(len(instructions.instructions)):
        ...
        if instructions.instructions[inst] == "G": #Go to front
            new_angle = radians(last[2])
            next = (last[0]+cos(new_angle)*line_size,last[1]+sin(new_angle)*line_size,degrees(new_angle))		
...
def GetInput(System): #Verificacao de eventos
        ...
        if event.type == QUIT or key_pressed[K_ESCAPE]:
            ...
            system = Instructions("F",{'F':'FF-[-F+F+F]+[+F-F-F]'},22.5,5) #Tree
            system.solve()
            zoom = 10
            local = [450,450,270]
            gerar(system,zoom,local)
            ...
            system = Instructions("F-G-G",{'F':'F-G+F+G-F','G':'GG'},120,10) #Sierpinski triangle
            system.solve()
            zoom = 10
            local = [450,450,270]
            gerar(system,zoom,local)
            ...
            system = Instructions("X",{'X':'F+[[X]-X]-F[-FX]+X','F':'FF'},25,10) #Sierpinski triangle
            system.solve()
            zoom = 10
            local = [450,450,270]
            gerar(system,zoom,local)
